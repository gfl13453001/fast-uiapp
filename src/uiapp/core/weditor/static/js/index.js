...
window.vm = new Vue({
  ...
  methods: {
    ...
    doSendKeys: function (text) {
      ...
      const code = `uiapp.send_keys("${text}", clear=True)`
      this.loading = true;
    ...
    doTapWidget() {
      ...
      $.ajax({
        ...
        const code = `uiapp.widget.click("${ret.id}#${ret.note}")`;
        this.codeInsert(code)
        this.nodeSelected = null;
        this.runPythonWithConnect(code)
          .then(this.delayReload)
    ...
    generateNodeSelectorCode: function (node) {
      if (this.useXPathOnly) {
        return `uiapp.xpath('${this.elemXPathLite}')`
      }
